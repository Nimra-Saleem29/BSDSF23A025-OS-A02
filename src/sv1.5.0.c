/* src/lsv1.5.0.c
 * ls v1.5.0 - colorized output based on file type
 *
 * Color rules:
 *   - Directory: Blue
 *   - Executable: Green (any exec bit set)
 *   - Tarballs (.tar, .tgz, .tar.gz, .gz, .zip): Red
 *   - Symbolic links: Magenta
 *   - Special files (char/block/socket/fifo): Reverse video
 *
 * Colors are only enabled when stdout is a terminal (isatty).
 */
...
void print_columns_down_across(const char *dirpath, char **names, int n) {
    ...
    for (int r = 0; r < nrows; ++r) {
        for (int c = 0; c < ncols; ++c) {
            ...
            if (idx >= n) continue;
            int last = (c == ncols - 1);
            print_colored_padded(dirpath, names[idx], colw, last);
        }
        printf("\n");
    }
}
...
void print_horizontal(const char *dirpath, char **names, int n) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        print_colored_padded(dirpath, names[i], colw, 0);
        cur += colw;
    }
    printf("\n");
}
